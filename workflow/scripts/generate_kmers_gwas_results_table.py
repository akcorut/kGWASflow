...
with open(snakemake.log[0], "w") as f:
    ...
    dir_path = snakemake.params.in_prefix # Input directory path for kmersGWAS results
    ...
    def parse_kmersGWAS_results(input_dir="", threshold=5):
        ...
        if threshold == 5:
            ...
            df_5per = pd.DataFrame(pd.Series(pheno_kmer_dict).reset_index()).set_axis(['pheno','kmers_pass_threshold_5per'],axis=1,inplace=False)
        ...
        if threshold== 10:
            ...
            df_10per = pd.DataFrame(pd.Series(pheno_kmer_dict).reset_index()).set_axis(['pheno','kmers_pass_threshold_10per'],axis=1,inplace=False)
    ...
    def get_phenos_with_sig_kmers(res_tab, threshold=5):
        ...
        res_tab_filt.to_csv(snakemake.params.out_prefix + "/phenos_with_kmers_pass_threshold_{thr}per.txt".format(thr=str(threshold)), index=False, sep="\t")

        phenos_with_sig_kmers=res_tab_filt["pheno"]
        phenos_with_sig_kmers.to_csv(snakemake.params.out_prefix + "/phenos_with_sig_kmers_list_{thr}per.txt".format(thr=str(threshold)), index=False, sep="\t")

    ## Parse pass_threshold_5per results and write out the output
    results_5per = parse_kmersGWAS_results(dir_path, threshold=5)
    results_5per.to_csv(snakemake.params.out_prefix + "/kmers_gwas_results_table_5per.txt", index=False, sep="\t")
    ## Get phenos with significant k-mers list and write out the output
    get_phenos_with_sig_kmers(results_5per, threshold=5)

    if (results_5per['num_kmers_pass_threshold_5per'] != 0).sum() == 0:
        with open(snakemake.params.out_prefix + "/NO_KMERS_PASS_5PER_THRESHOLD_FOUND", 'w'): pass

    ## If 10 per threshold activated use the results from pass_threshold_10per
    if snakemake.config["params"]["results_table"]["use_10_per"]:
        results_10per = parse_kmersGWAS_results(dir_path, threshold=10)
        results_10per.to_csv(snakemake.params.out_prefix + "/kmers_gwas_results_table_10per.txt", index=False, sep="\t")
        get_phenos_with_sig_kmers(results_10per, threshold=10)
        if (results_10per['num_kmers_pass_threshold_10per'] != 0).sum() == 0:
            with open(snakemake.params.out_prefix + "/NO_KMERS_PASS_10PER_THRESHOLD_FOUND", 'w'): pass